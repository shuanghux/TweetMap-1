...
function showSentiment(){
  if (isSentiment) {d3.selectAll("svg").remove();}
      ...
      for (var i in jsonArr) {
            ...
            var polarity = currObj["polarity"];
            json["countries_msg_vol"][country_code]["tweets"] += 2;
            json["countries_msg_vol"][country_code]["polarity"] += polarity;
      }
      // D3 Bubble Chart 
    ...
    function processData(data) {
      ...
      for(var prop in obj) {
        newDataSet.push({name: prop, className: prop.toLowerCase(), size: obj[prop]["tweets"], polarity: obj[prop]["polarity"]+1});
      }
